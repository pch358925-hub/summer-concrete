...
- 현장명 기본값: `세종천안 2공구 (주)한화`
- 타설부위 직접 입력
- 타설부위 예시: `금단교 A1(세종) 벽체 1단`
- 사진대지 목록
- 월별 사진대지 목록 필터
- 1일차~5일차 사진 등록 상태 확인
- 사진 업로드 시 자동 JPEG 압축
...
- 출력 행 이름: `위치`, `사진내용`
- 출력표 크기: `152mm x 210mm`
- 사진 출력 크기: `149mm x 82mm`
...
5. 상단 `사진대지 목록`에서 타설일별 등록 현황을 확인합니다.
6. `인쇄/PDF`를 눌러 사진대지를 출력합니다.

## 사진 용량

원본 사진을 그대로 저장하지 않고, 업로드 전 브라우저에서 긴 변 기준 최대 1600px 수준으로 줄이고 JPEG 품질을 낮춰 저장합니다. 출력 사진 크기 `149mm x 82mm` 기준으로는 현장 출력물에 충분한 수준입니다.
